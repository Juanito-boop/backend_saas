...
export const products = pgTable(
  'products',
  {
    ...
    scrapeStrategy: varchar('scrape_strategy', { length: 30 })
      .default('browser')
      .notNull(),
...
export const priceHistory = pgTable(
  ...
  (table) => [index('idx_price_history_product_id').on(table.productId)],
);

export const priceHistoryHourly = pgTable(
  ...
  (table) => [
    uniqueIndex('price_history_hourly_product_bucket_unique').on(
      table.productId,
      table.bucketStart,
    ),
...
export const priceHistoryDaily = pgTable(
  ...
  (table) => [
    uniqueIndex('price_history_daily_product_bucket_unique').on(
      table.productId,
      table.bucketStart,
    ),
...
export const subscriptions = pgTable(
  ...
  (table) => [index('idx_subscriptions_team_id').on(table.teamId)],
);
...
export const notificationWebhookDeliveries = pgTable(
  'notification_webhook_deliveries',
  {
    ...
    webhookId: uuid('webhook_id')
      .references(() => notificationWebhooks.id)
      .notNull(),
    teamId: uuid('team_id')
      .references(() => teams.id)
      .notNull(),
  ...
  (table) => [
    ...
    index('idx_notification_webhook_deliveries_notification_id').on(
...
export const activityLog = pgTable(
  ...
  (table) => [index('idx_activity_log_team_id').on(table.teamId)],
);
